...
#define iDz iD+2*no
    ...
    if ((ix<nmp)&&(iy<nn)&&(mn[iD]>(DAT)0.0)){
        ...
        atomicAdd(&un[iDz],prod*vp[ix+2*nmp]*bc[iDz]);
    }
#undef iD    
#undef iDx
#undef iDy
...
__global__ void swapBCs(int* bcNew, int* bcOld, int no){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
    if(ix<no){
        bcOld[ix+0*no] = bcNew[ix+0*no];
        bcOld[ix+1*no] = bcNew[ix+1*no];
        bcOld[ix+2*no] = bcNew[ix+2*no];
    }
}
