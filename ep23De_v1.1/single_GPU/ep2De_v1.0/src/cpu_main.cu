...
int main(){
    ...
    while(tw<t){
        ...
        g  = getG(tw,tg);
        // 
        topol(xp_h,p2e_h,p2n_h,e2n_h,xnmin,ynmin,dx,dy,nmp,nn,ney,nel);
        basis(xp_h,xn_h,N_h,dNx_h,dNy_h,p2n_h,lp_h,dx,dy,nmp,nn,no);
        accum(mn_h,pn_h,fen_h,fin_h,N_h,dNx_h,dNy_h,mp_h,vp_h,sig_h,vol_h,p2n_h,g,nmp,nn,no);
        ...
        strains(un_h,dNx_h,dNy_h,dF_h,eps_h,ome_h,lp_h,vol_h,p2n_h,nmp,nn,no);
        elast(sig_h,eps_h,ome_h,Del_h,nmp,dt);
        if(tw>te){
            plast(sig_h,cohp_h,phip_h,epII_h,iDel_h,Hp,cohr,nmp);
        }
    ...
    // save data
    save(epII,nmp  ,1,"C_epII.dat");
    save(xp  ,2*nmp,1,"C_xp.dat"  );
    save(lp  ,2*nmp,1,"C_lp.dat"  );
    save(up  ,2*nmp,1,"C_up.dat"  );
    save(sig ,4*nmp,1,"C_sig.dat" );

FILE* fidw=fopen("CPUtime.dat", "wb");
fwrite(CPUinfo, 2*sizeof(DAT), 1, fidw);
fclose(fidw);

save(pn ,no,2,"matrix.dat" );
