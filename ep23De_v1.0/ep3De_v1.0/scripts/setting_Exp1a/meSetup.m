function [meD,bc] = meSetup(nEx,length,height,width,typeD)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
%% MESH INITIALIZATION
meD.vd  = 0.1                                                             ;% viscous damping coefficient
meD.h   = [length/nEx length/nEx length/nEx]                              ;% [dx dy]
[x,z,y] = meshgrid(0.0-2*meD.h(1):meD.h(1):4*length+2*meD.h(1),...
                   0.0-2*meD.h(3):meD.h(3):1*height+2*meD.h(3),...
                   0.0-2*meD.h(2):meD.h(2):width+2*meD.h(2))           ;%
x      = flip(x)                                                          ;%
z      = flip(z)                                                          ;%
y      = flip(y)                                                          ;%               
  
meD.L   = [max(x(:))-min(x(:)),max(y(:))-min(y(:)),max(z(:))-min(z(:))]     ; 
