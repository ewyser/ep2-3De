...
% mesh
% -------------------------------------------------------------------------%
clear,close,fig3=figure(3);clf,fig4=figure(4);clf                                                               ;%
run('preprocessing');%
run('compileExp2a')

lx    = 64.1584                                                           ;% mesh size along x direction
%Ly    = [lx]./[64 32 16 8 4 2 1 1/2 1/4 1/8 1/16 1/32]                    ;% mesh size along y direction
Ly    = [lx]./[8];
numel = [80]       %80                                                       ;% -
for sim=1:length(Ly)
    disp('------------------------')                                      ;%
    disp(['Run ',num2str(sim),': nel = ',num2str(numel(1)),''])           ;%
    disp('------------------------')                                      ;%
    ...
    [meD,bc]= meSetup(numel(1),lx,Ly(sim),lz,typeD)                       ;% - see function
    ...
    GPU(:,sim) = [mpD.n;GPUtime];
    fid  = fopen('CPUinfo.dat'); CPUtime = fread(fid,typeD); fclose(fid);
    CPU(:,sim) = [mpD.n;CPUtime];
...
name = name.Name
name(name == ' ') = []
name=['gpuPerf' name '.mat'];
save(name,'GPU','-v7.3');
name=['cpuPerf_i910900K.mat'];
save(name,'CPU','-v7.3');
