...
nvcc ../../src/gpu_main.cu -o gpu.out -arch=sm_70 --compiler-options -O3 --compiler-bindir mpic++ -DSIM=$1 -DDAT=$2 -DFPS=$3 -DD=$4 -DBCK1=$5 -DBCK2=$6 -DDIMS_X=$7 -DBFS=$8
#------------------------------------------------------------------
# run GPU code 
run_cmd="-np $nprocs -rf ../../rkf/gpu_rankfile_volta ./gpu.out"
echo $mpirun $run_cmd
$mpirun $run_cmd
#------------------------------------------------------------------
## nvidia profiler
